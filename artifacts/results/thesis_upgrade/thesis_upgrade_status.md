...
| Neuromaps spatial nulls | implemented_schaefer100_receptor_myelin_gradient_moran_spatial_nulls | false | 0.70 | Partial receptor/myelin/gradient Schaefer100 Moran spatial nulls are run; full family coverage is still missing. |
...
| Full neuromaps spatial-autocorrelation nulls | implemented_schaefer100_receptor_myelin_gradient_moran_spatial_nulls | false | Schaefer100 receptor, myelin, and functional-gradient Moran spatial nulls are executed, but gene-expression and surface-level family coverage is still missing. | Add gene-expression maps and surface-level null coverage in the active high-resolution/surface space, then rerun the same Moran/spatial-null plus FDR gate family. |
