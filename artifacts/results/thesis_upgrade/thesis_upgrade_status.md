...
| Neuromaps spatial nulls | implemented_partial_receptor_schaefer100_moran_spatial_nulls | false | 0.70 | Partial receptor-only Schaefer100 Moran spatial nulls are run; full family coverage is still missing. |
...
| Full neuromaps spatial-autocorrelation nulls | implemented_partial_receptor_schaefer100_moran_spatial_nulls | false | Receptor-only Schaefer100 Moran spatial nulls are executed, but full myelin, gradient, gene-expression, and surface-level family coverage is missing. | Add myelin, functional-gradient, and gene-expression maps in the active high-resolution/surface space, then rerun the same Moran/spatial-null plus FDR gate family. |
