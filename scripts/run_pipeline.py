...
STAGE_2_HELDOUT_SOBER_TARGET = (
    REPO_ROOT / "results" / "stage_2" / "heldout_validation" / "empirical_sober_targets.yaml"
)
STAGE_2_HELDOUT_PERTURBATION_TARGET = (
    REPO_ROOT / "results" / "stage_2" / "heldout_validation" / "empirical_perturbation_targets.yaml"
)
...
def _subject_split_is_approved(subject_split_path: str | Path | None) -> bool:
...
def _stage_3_needs_stage_2(subject_split_path: str | Path | None) -> bool:
    if not STAGE_2_SOBER_TARGET.exists() or not STAGE_2_PERTURBATION_TARGET.exists():
        return True
    if _subject_split_is_approved(subject_split_path):
        return (
            not STAGE_2_HELDOUT_SOBER_TARGET.exists()
            or not STAGE_2_HELDOUT_PERTURBATION_TARGET.exists()
        )
    return False
...
def run_stage(
    ...
    if stage == "stage1":
        ...
        resolved_stage2_output = REPO_ROOT / "results" / "stage_2" if stage2_output_dir is None else Path(stage2_output_dir)
        legacy_stage2_output = (REPO_ROOT / "results" / "stage_2").resolve()
        if include_music and resolved_stage2_output.resolve() == legacy_stage2_output:
            raise ValueError(
                "run-02 music extraction must use an explicit non-legacy --stage2-output-dir, "
                "for example results/setting_seed/run02_extraction/stage_2_music."
            )
        report_path = (
            REPO_ROOT / "docs" / "stage_reports" / "stage_2.md"
            if resolved_stage2_output.resolve() == legacy_stage2_output
            else resolved_stage2_output / "stage_2_report.md"
        )
        generate_stage_2_outputs(
            graph_path=REPO_ROOT / "configs" / "graphs" / "macro_modules.yaml",
            baseline_path=REPO_ROOT / "configs" / "regimes" / "baseline.yaml",
            target_path=REPO_ROOT / "configs" / "targets" / "sober_summary_targets.yaml",
            output_dir=resolved_stage2_output,
        ...
        approved_split = _subject_split_is_approved(subject_split_path)
        if _stage_3_needs_stage_2(subject_split_path):
            print("[pipeline] stage3 needs stage2 empirical targets; running stage2 first", flush=True)
            run_stage("stage2", subject_split_path=subject_split_path)
        generate_stage_3_outputs(
            ...
            seed=11,
            subject_split_path=subject_split_path,
            heldout_sober_target_path=STAGE_2_HELDOUT_SOBER_TARGET if approved_split else None,
            heldout_perturbation_target_path=(
                STAGE_2_HELDOUT_PERTURBATION_TARGET if approved_split else None
            ),
        )
    elif stage == "stage4":
        if not STAGE_2_SOBER_TARGET.exists() or not STAGE_2_PERTURBATION_TARGET.exists():
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        default="bistable",
        help="Model family to use. Stage 1-4 commands currently default to the legacy bistable baseline.",
    )
    parser.add_argument(
        "--parcellation",
        ...
        help="Empirical target parcellation for generate-empirical-targets.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Write metadata/plan only for commands that support dry runs.",
    )
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Use the smallest deterministic development budget for new fitting commands.",
    )
    parser.add_argument(
        "--subject-split",
        default=None,
        help="Path to a JSON subject-disjoint split file for Stage 2/3 scaffold metadata.",
    )
    parser.add_argument(
        "--include-music",
        action="store_true",
        help="Explicitly allow ds003059 run-02 music extraction. Requires a non-legacy --stage2-output-dir.",
    )
    parser.add_argument(
        "--runs",
        nargs="+",
        default=None,
        choices=["run-01", "run-02", "run-03"],
        help="Stage 2 ds003059 run list. Defaults to run-01 run-03 unless --include-music is used.",
    )
    parser.add_argument(
        "--stage2-output-dir",
        default=None,
        help="Optional Stage 2 output directory. Required for --include-music to avoid overwriting legacy results/stage_2.",
    )
    ...
    stage2_output_dir = Path(args.stage2_output_dir) if args.stage2_output_dir is not None else None
    stage2_runs = tuple(args.runs) if args.runs is not None else None
