...
STRIATAL_TOKENS = ("striat", "caudate", "putamen", "pallidum", "accumbens")
...
def infer_module_prior(module: str, prior: str) -> float:
    ...
    if prior == "striatum":
        return 1.0 if any(token in key for token in STRIATAL_TOKENS) else 0.0
    return 0.0
...
def module_masks(modules: tuple[str, ...]) -> dict[str, np.ndarray]:
    ...
    striatum = np.asarray([infer_module_prior(module, "striatum") > 0.0 for module in module_list], dtype=bool)
