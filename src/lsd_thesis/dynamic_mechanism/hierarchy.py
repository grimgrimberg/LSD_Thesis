...
from lsd_thesis.dynamic_mechanism.connectivity import (
    mean_between,
    mean_within_networks,
    safe_vector_correlation,
)
from lsd_thesis.dynamic_mechanism.priors import (
    infer_module_prior,
    module_masks,
...
def _hierarchy_routing_metrics(modules: tuple[str, ...], time_series: np.ndarray) -> dict[str, float]:
    ...
    striatal_sensory = mean_between(fc_matrix, masks["striatum"], masks["sensory"])
    ...
    return {
        ...
        "striatal_sensory_coupling": striatal_sensory,
...
def summarize_hierarchy_routing(pairs: Sequence[Any]) -> dict[str, Any]:
    ...
    has_striatal_node = bool(
        pairs and any(infer_module_prior(module, "striatum") > 0.0 for module in pairs[0].modules)
    )
    if has_striatal_node:
        metric_names.append("striatal_sensory_coupling")
    ...
    expected_direction = {
        ...
        "striatal_sensory_coupling": "positive means stronger striatal coupling with sensory/unimodal modules under LSD",
